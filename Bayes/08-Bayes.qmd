...
-   Follow along: <https://github.com/suyoghc/PSY-504_Spring-2025/blob/main/Bayes/08-Bayes.qmd>
...
| :----------------------- | :------------------------------------ | :------- |
| **Probability philosophy** | Long-run frequency                    |          |
| **Parameters** | Fixed unknowns                        |          |
| **Estimation** | Point estimates (MLE)                 |          |
| **Uncertainty** | Confidence intervals                  |          |
| **Interpretation** | "95% of samples contain true value" |          |
| **Prior Info** | Not formally used                     |          |
| **Prediction** | Points with errors                    |          |
| **Testing** | p-values                              |          |
| **Comparison** | AIC, BIC                              |          |
| **Computation** | Often analytical                      |          |
| **Workflow** | Residuals, QQ plots                   |          |
| **Small Samples** | Often poor                            |          |
| **R Implementation** | lm(), glmer()                         |          |
| **Speed** | Faster                                |          |
| **Regularization** | Penalties (LASSO)                     |          |
:::
...
- Experiment design
- Power analysis
- ---------------------------------> _Prior specification; Prior predictive checks_
- Model specification
- Model Fitting
- Model Checking
...
-   $\hat{R}$
    ...
    -   It compares within-chain and between-chain variance. If chains explore the same areas, R-hat approaches 1.

    -   Ratio of variance (like *F* test)
    
    -   0.99 to 1.01 are considered acceptable (across all parameters)
...
-   Effective sample size (ESS)
    ...
    -   Autocorrelation of samples inevitable (each sample is influenced by the previous)
...
-   Good prior predictive checks validate that your model makes sense conceptually - not that it predicts your specific data accurately. 
    - That's what posterior checks are for.

-   Make sure prior distribution makes sensible predictions (if using informative priors)
    - Predictions cover reasonable ranges
    - Values are plausible (not theoretically impossible)
    - Distribution shape is sensible
    
-   Prior predictions should be wider than posterior predictions. 
    - They represent uncertainty before data.
    - You don't want overconfident priors
...
- Complete pooling
  ...
  - Con: Misses group variation

- No pooling
  - Pro: Captures group uniqueness
  - Con: Limited by group sample size
...
- What is shrinkage?
    - Estimates "shrink" toward a common value
    - Groups share information
    - Small samples borrow strength
    - Outliers get pulled toward average
