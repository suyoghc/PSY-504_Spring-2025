...
plot_race <- ggplot(voter_data, aes(x = race, fill = voter_category)) +
      ...
      labs(x = "Race", fill = "Voter Category") +
      theme_minimal()
...
```{r}
# Enter code
plot_education <- ggplot(voter_data, aes(x = educ, fill = voter_category)) +
      geom_bar(position = "fill") +
      labs(x = "Education", fill = "Voter Category") +
      theme_minimal()
plot_education
```
...
library(patchwork)
# Enter code
combined <- plot_race + plot_education
combined
```
...
```{r}
library(nnet)
model1 <- multinom(voter_category ~ ppage + race + gender + income_cat + educ, 
                        data = voter_data)
```
...
```
    ...
    > #Enter answer based on your code: Yes!
...
```{r}
multinomial_analysis2 <- emmeans(model2, ~ educ | voter_category)

coefs = contrast(regrid(multinomial_analysis2, "log"),"trt.vs.ctrl1",  by="educ")
...
```{r}
  ...
  pp_partyID <- ggemmeans(model2, terms = "pol_ident_new")
      ggplot(pp_partyID, aes(x = x, y = predicted, fill = response.level)) +
      geom_bar(stat = "identity", position = "dodge") +
      labs(x = "\nParty ID", y = "Predicted Probablity\n", title = "Predicted Probabilities of Voting Frequency by Party ID")+
      scale_fill_manual(
        name = NULL,
        values = c("always" = "#F6B533", "sporadic" = "#D07EA2", "rarely/never" = "#9854F7"),
        labels = c("RARELY OR NEVER VOTE    ", "SOMETIMES VOTE    ", "ALMOST ALWAYS VOTE    "),
...
pp_ed <- ggemmeans(model2, terms = "educ")

  ggplot(pp_ed, aes(x = x, y = predicted, fill = response.level)) +
      geom_bar(stat = "identity", position = "dodge") +
    ...
    theme_minimal()
```
Main takeaways from the graphs above: As age increases the probability of "rarely or never voting" decreases, the probability of "sometimes voting" and "rarely or never voting" is around the same for both democratic and republican political ID, as education level increases, the probability of "sometimes voting" increases, while the probability of "rarely or. never voting" increases with less education one has.
...
Enter your interpretation here: People with high school or less education are less likely to vote sporadically compared to those who have a college level education. Those with some college are less likely to vote sporadically than those with college level education. Individuals with some college education are more likely to vote sporadically than those with a high school or less education. Those with a high school or less education are less likely to always vote compared to those with a college education. On a similar note, those with some college education are less likely to always vote compared to those with a college education. Finally, those with some college education are more likely to always vote compared to those with a high school or less education. Altogether, it seems like those with a greater level of education are more likely to always vote compared to voting sporatically and rarely/never. 
